...
export default function Dashboard({ borrower, loading, error }) {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[400px_1fr] gap-6">
      ...
      {/* ── Left Column: Score + Coaching + API ── */}
      <div className="flex flex-col gap-8">
        ...
        <div className="glass-card p-5 animate-fade-in-up-delay-3 flex flex-col justify-center">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[11px] font-semibold text-surface-400 uppercase tracking-widest flex items-center gap-2">
              <span className="font-mono text-surface-500">{`{ }`}</span> API Response (SCORERESPOND)
            </h3>
            <div className="flex gap-2">
              <button 
                onClick={() => setShowJson(!showJson)}
                className="text-[10px] bg-[rgba(255,255,255,0.03)] hover:bg-[rgba(255,255,255,0.08)] border border-[rgba(255,255,255,0.05)] px-2.5 py-1.5 rounded transition-colors"
              >
          ...
          {showJson && (
            <div className="mt-2 bg-[#050810] border border-[rgba(255,255,255,0.03)] rounded-lg p-4 overflow-auto max-h-60 custom-scrollbar">
          ...
          {!showJson && (
            <div className="mt-1 flex items-center gap-2 text-surface-500 font-mono text-[10px]">
